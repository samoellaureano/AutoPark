...
def findPlace(contornos, imagem):
    print("Captura de contornos")
    for c in contornos:
        # perimetro do contorno, verifica se o contorno é fechado
        perimetro = cv2.arcLength(c, True)
        if perimetro > 200 and perimetro < 900:
...
def reconhecimentoOCR(path_img):
    ...
    # img = cv2.erode(img,kernel,1,1,cv2.BORDER_REFLECT)
...
while (video.isOpened()):
    ...
    # desfoque
    img_result = cv2.GaussianBlur(img_result, (3, 3), 3)

    # limiarização
    ret, img_result = cv2.threshold(img_result, 80, 255, cv2.THRESH_BINARY)
    img_result = cv2.Canny(img_result, 100, 20)

    # lista os contornos
    img, contornos, hier = cv2.findContours(img_result, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
